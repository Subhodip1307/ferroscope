use ferroscope_server::global::structure::{BGRulesData, NotificationChannel, NotificationData};
use lettre::{
...
async fn notifier_worker(
    ...
    tokio::spawn(async move {
        while let Some(msg) = receiver.recv().await {
            node_notifier(msg,&pg_pool,&mail_sender).await;
        } //end while of mpsc recv
    }); //end tokio::spwn
}

async fn node_notifier(
...
async fn mail_sender_worker(mut receiver: mpsc::Receiver<EmailBody>) {
