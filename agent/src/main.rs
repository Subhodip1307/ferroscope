    ...
    loop {
        while let Some(payload)=receiver.recv().await{
            ...
            match api_client.post(payload.endpoint).json(&payload.body).send().await{
                Ok(status)=>println!("the status code is {}",status.status()),
                Err(e)=>{println!("getting error {e}, going to sleep");tokio::time::sleep(std::time::Duration::from_secs(60)).await;println!("Sleeping done!")}
            }
        }
    }
}