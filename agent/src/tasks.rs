use crate::{Payload, jobs, set_up, set_up::BaseConFig, system, system::logic};
use reqwest::Client;
...
pub async fn spawn_info_sender(
    conf: Arc<BaseConFig>,
            ...
            loop {
                ...
                system_api_sender
                    .send(Payload {
                        ...
                        body: json!(logic::get_disk_io().await),
                    })
                    .await
                    .unwrap();
                tick.tick().await;
    ...
    loop {
        let _ = api_client
            .get(&format!("{}/helth_check", system_conf.get_server_url()))
            .send()
            .await;
        tick.tick().await;
    }
    ...
    loop {
        while let Some(payload) = receiver.recv().await {
                ...
                .await
            {
                ...
                Err(e) => {
                    println!("getting error {e}, going to sleep");
